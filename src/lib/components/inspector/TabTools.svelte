<script lang="ts">
  ...
  // Реактивная валидация: пересчитывается при каждом изменении configBuffer
  ...
  // Проверка изменений и возможности сохранения
  const isDirty = $derived(configBuffer !== (currentWorkspace.settings.mcpConfig || ''));
  const canSave = $derived(isDirty && !jsonError);

  function validateAndSave() {
    // Сохраняем, так как canSave гарантирует валидность JSON
    currentWorkspace.settings.mcpConfig = configBuffer;
    isSaved = true;
    onSettingsChange();
    
    // Закрываем через небольшую паузу после успеха для визуального фидбека
    setTimeout(() => { 
      isSaved = false; 
      isExpanded = false; 
    }, 600);
  ...
  function handleCancel() {
    // Сброс буфера к сохраненному значению и закрытие
...
<div class="tab-tools">
  ...
  <button class="expand-trigger" onclick={() => isExpanded = true}>
    {@html ExpandIcon}
...
<style>
  ...
  .info-text { font-size: 0.85rem; color: #6b7280; line-height: 1.4; }

  .expand-trigger {
    display: flex; align-items: center; justify-content: center; gap: 8px;
    padding: 10px; background: #ffffff; border: 1px solid #e5e7eb;
    border-radius: 8px; cursor: pointer; color: #374151; font-weight: 500;
    transition: all 0.2s; margin-top: 4px;
  }
  .expand-trigger:hover { background: #f9fafb; border-color: #5865f2; color: #5865f2; }
  .expand-trigger :global(svg) { width: 14px; height: 14px; }
  ...
  .primary-btn {
    padding: 10px 24px; background: #2f2f2f; color: white; border: none;
