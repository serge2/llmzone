<script lang="ts">
  ...
  // --- ЛОГИКА ВИДЖЕТОВ ---
  let serverMap = new Map<string, MCPServerInstance>();

  const activeServers = $derived.by(() => {
    try {
      const config = JSON.parse(currentWorkspace.settings.mcpConfig || '{}');
      ...
      for (const [name, info] of Object.entries(servers)) {
        ...
        if (!serverMap.has(key)) {
          // Передаем хедеры при создании
          serverMap.set(key, new MCPServerInstance(name, url, headers));
        } else {
          // Обновляем хедеры у существующего инстанса
          const instance = serverMap.get(key);
          if (instance) instance.headers = headers;
        }
      }
      ...
      console.error("MCP Widget parse error:", e);
      return [];
    }
  });
...
<div class="tab-tools">
  <div class="header-row">
    <span class="label-text">Конфигурация MCP (SSE)</span>
  ...
  {#if activeServers.length > 0}
    <div class="servers-list">
      {#each activeServers as server (server.name + server.url)}
    ...
    >
      ...
      <div class="panel-body">
        <div class="editor-top-bar">
          <span class="hint">На данный момент есть поддержка только HTTP(S) SSE</span>
          {#if jsonError}
...
<style>
  ...
  .config-btn {
    ...
    border-radius: 6px; cursor: pointer; color: #6b7280; font-size: 0.75rem;
  ...
  .overlay-backdrop {
    position: absolute;
    top: 0; left: 0; right: 100%; width: 200vw; height: 100%;
    background: rgba(15, 23, 42, 0.15); backdrop-filter: blur(2px); z-index: 800;
  }

  .expanded-editor-panel {
    position: absolute;
    top: 0; right: 100%; width: calc(100vw - 380px - 40px); height: 100%;
    background: #ffffff; z-index: 850; display: flex; flex-direction: column;
    box-shadow: -15px 0 30px rgba(0, 0, 0, 0.05); border-left: 1px solid #e5e7eb;
  }
  ...
  .mcp-textarea {
    ...
    font-family: 'ui-monospace', monospace; font-size: 0.9rem; line-height: 1.6;
    padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px;
    background: #f8fafc; color: #334155; outline: none; resize: none; transition: all 0.2s;
  }
