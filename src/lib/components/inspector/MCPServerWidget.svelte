<script lang="ts">
  ...
  // Иконки (можно заменить на ваши svg)
  ...
  // Состояние для отображения описания конкретного инструмента
  let activeTooltip = $state<string | null>(null);

  // Режим управления: true - разрешить все, false - индивидуальное управление
  let autoApproveAll = $state(true);
  ...
  // Принудительная переинициализация подключения
...
<div class="server-card" class:is-loading={server.isLoading}>
  <div class="card-main">
    <button 
      ...
      onclick={() => server.isExpanded = !server.isExpanded}
    >
      {@html ChevronIcon}
    </button>
        ...
        <div class="tools-list">
          <div class="section-header">
            ...
            <div class="global-policy">
              <span class="policy-label">{autoApproveAll ? 'Разрешить все' : 'По-инструментное управление'}</span>
              <button 
                class="policy-toggle" 
                onclick={() => autoApproveAll = !autoApproveAll}
                class:is-active={autoApproveAll}
          ...
          {#each server.tools as tool}
            <div class="tool-item">
              ...
              <div class="tool-actions">
                {#if !autoApproveAll && tool.enabled}
                  <button 
                    class="auth-badge" 
                    class:is-trusted={tool.alwaysAllow}
                    onclick={() => tool.alwaysAllow = !tool.alwaysAllow}
...
<style>
  .server-card {
    ...
    border-radius: 10px;
    overflow: visible; /* Важно для тултипов */
    transition: all 0.2s;
    position: relative;
  }
  ...
  .expand-btn {
    ...
    display: flex; transition: transform 0.2s; padding: 4px;
  }
  ...
  .slider {
    position: absolute; cursor: pointer; inset: 0; background-color: #e5e7eb;
  ...
  /* Глобальная политика подтверждения */
  .global-policy {
    display: flex;
    align-items: center;
    gap: 8px;
  ...
  .policy-toggle.is-active .toggle-track {
  ...
  .tool-item { 
    ...
    background: white; padding: 6px 10px; border-radius: 6px; border: 1px solid #f1f5f9;
  }
